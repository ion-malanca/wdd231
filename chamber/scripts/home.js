...
function displayCurrent(data) {
    ...
    document.getElementById("current-weather").innerHTML = `
        ...
        <p>Sunrise: ${new Date(data.city.sunrise * 1000)
            .toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit'
            })}
        </p>
        <p>Sunset: ${new Date(data.city.sunset * 1000)
            .toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit'
            })}
        ...
        forecastDiv.innerHTML += `
            <div class="forecast-day">
                <p>${date.toLocaleDateString("en-US", {
                    weekday: "long"
                })}: <strong>${Math.round(item.main.temp)} °C</strong></p>
            </div>
        `;
    }
}
...
async function loadCompanies() {
  try {
    ...
    companies.forEach(company => {
        if (company.membership < 2) return;
        const card = document.createElement("section");
